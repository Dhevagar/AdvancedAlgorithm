...
class SlowGram:
    ...
    def setup_data(self):
        p1 = Person("Dhevagar", "P", "Traveler and photographer.")
        p2 = Person("Jay Teh", "U", "Tech enthusiast and gamer.")
        p3 = Person("Han Yi", "P", "Food blogger and coffee lover.")
        p4 = Person("Jonathan", "P", "Fitness trainer and motivator.")
        ...
        self.graph.add_edge(p1, p3)
        self.graph.add_edge(p1, p4)
        self.graph.add_edge(p2, p1)
        self.graph.add_edge(p3, p4)
        self.graph.add_edge(p4, p5)
        self.graph.add_edge(p5, p1)
    ...
    # (b) View profile (with privacy control)
    def display_profile_with_privacy(self, index):
        people = self.people
        if 0 <= index - 1 < len(people):
            ...
            if person.get_privacy() == 'P':  # Public profile
                print(f"Biography: {person.get_biography()}")
                print("(Public profile — all details are visible.)")
            else:  # Private profile
                print("(This profile is private. Other details are hidden.)")
            print("-----------------------------------------")
        else:
    ...
    def run(self):
        ...
        while choice != "5":
            ...
            if choice == "1":
                ...
                self.display_profile_with_privacy(index)
